...
export default function StreaksScreen() {
  const { user } = useAuth();
  const [habbits, setHabbit] = useState<Habbit[]>([]);
  ...
  const habbitStreaks = habbits.map((habit) => {
    const { streak, bestStreak, total } = getStreakData(habit.$id);
    return { habit, bestStreak, streak, total };
  });

  const rankedHabits = habbitStreaks.sort(
  ...
  return (
    <View style={styles.container}>
      ...
      {habbits.length === 0 ? (
        <View>
          <Text>No Habbit Yet. Add your first Habbit</Text>
        </View>
      ) : (
